		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="mui-table-view-cell mui-media">
					...
					<div style="margin-top:0.2em;">
						...
						<span style="float:right;"> 
							[ <a href="javascript:;" onclick="cancel('/house/index.php/Collect/cancel/id/<?php echo ($vo["id"]); ?>')">取消收藏</a> ] 
						</span> <br>
						
						收藏时间 <?php echo date('Y-m-d H:i:s',$vo['add_time']);?>						
					</div>
...
<script>
    ...
    function cancel(url){
	 	var btnArray = ['否', '是'];
	    mui.confirm('你真的要取消收藏吗？', '提示', btnArray, function(e) 
	    {
	        if (e.index == 1) {
	            // console.log(1);
				$.post(url, {},
				    function(data){
				      ...
				      if( data.status==0 ){
				      	...
				      	mui.toast(data.message);
				      	setTimeout(function(){
				      		window.location="/house/index.php/Collect";
				      	},2000);
				      }
